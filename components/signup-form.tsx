  ...
  async function onSubmit(values: any) {
    ...
    if (!data || error) {
      ...
      toast.success("Account created successfully", {
        description:
          "Kindly check your email for verification. Check spam folder as well!",
      });
    }
    setIsLoading(false);
  }
      ...
      <div className="w-full max-w-md bg-amber-50/2 rounded-lg p-4 space-y-6">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-2">
            ...
            <Button
              type="submit"
              className="flex flex-row w-full justify-center items-center h-12 bg-blue-600 text-white hover:scale-101 transition-all duration-200 easy-in-out"
              disabled={isLoading}
            >
              {isLoading ? (
                <span className="flex flex-row justify-center items-center w-full">
                  Creating... <Spinner className="w-4 h-4 absolute ml-2" />
                </span>
              ) : (
        ...
        <div className="items-center py-2 text-center">
          <span className="text-base z-10  px-2 text-muted-foreground">
            ...
            <a
              ...
              className="underline text-blue-500 hover:font-semibold underline-offset-4"
            >
              Login
            </a>
          </span>
